            ...
            <div class="result-content">
              ...
              <div class="result-book-info">
                <n-button
                  ...
                  :disabled="!result.bookId ? true : undefined"
                  class="book-link"
                >
                  <template #icon>
                    <n-icon>
                  ...
                  {{ result.bookId ? `จากหนังสือ: ${result.bookName}` : `จากโอวาทเรียบเรียง: ${result.bookName}` }}
                </n-button>
              </div>
              ...
              <div class="result-actions">
                <n-space>
                  ...
                  <n-button
                    v-if="result.chapterLinkPdf && result.chapterLinkPdf.includes('.pdf')"
    ...
    >
      <div v-if="detailModal.content" class="detail-content">
        <div class="detail-header">
          ...
          <div class="detail-book-info">
            <n-button
              ...
              :disabled="!detailModal.content.bookId ? true : undefined"
              class="book-link"
            >
              <template #icon>
                <n-icon>
              ...
              {{ detailModal.content.bookId ? 
                `จากหนังสือ: ${detailModal.content.bookName}` : 
                `จากโอวาทเรียบเรียง: ${detailModal.content.bookName}` 
              }}
            </n-button>
          </div>
        </div>
        ...
        <div class="detail-text" ref="detailTextRef">
          <n-highlight
            :text="detailModal.content.chapterDetail || detailModal.content.mark_details"
            :patterns="searchKeywords"
          />
        </div>
      </div>
    ...
    >
      <div v-if="youtubeModal.videoId" class="youtube-content">
        <div class="youtube-embed">
          <iframe
            :src="`https://www.youtube.com/embed/${youtubeModal.videoId}?autoplay=1&start=${youtubeModal.startTime}`"
            frameborder="0"
...
console.log('IndexsV2 (Naive UI) component loaded!')
...
const loadMoreResults = async (): Promise<void> => {
  ...
  try {
    ...
    console.log('More results loaded:', {
      newTotal: searchResults.value.length,
      remaining: totalResults.value - searchResults.value.length,
...
const navigateToBook = (bookId: string): void => {
  ...
  // Track analytics
  analytics.trackEvent('view_book_from_search', {
...
const getTimeFromURL = (url: string): number => {
...
const showYouTubeModal = (url: string): void => {
  ...
  if (videoId) {
    ...
    youtubeModal.value.startTime = getTimeFromURL(url)
  }
}
...
const closeYouTubeModal = (): void => {
  ...
  youtubeModal.value.startTime = 0
}
...
// Lifecycle
onMounted(async () => {
  // Get search keywords from store if available
  const existingSearch = searchStore.words_search
  if (existingSearch && existingSearch.length > 0) {
    searchKeywords.value = existingSearch.map(w => w.text || w)
    hasSearched.value = true
  }

  console.log('IndexsV2 mounted with creator:', creatorId.value)
})
</script>
